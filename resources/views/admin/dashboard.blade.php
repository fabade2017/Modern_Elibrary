...
<style>
    ...
    .stats-card.users { background: linear-gradient(135deg, #3b82f6, #93c5fd); }
    .stats-card.classes { background: linear-gradient(135deg, #10b981, #6ee7b7); }
    .stats-card.groups { background: linear-gradient(135deg, #f59e0b, #fcd34d); }
    .stats-card.categories { background: linear-gradient(135deg, #8b5cf6, #c4b5fd); }
    .stats-card.resources { background: linear-gradient(135deg, #ef4444, #fca5a5); }
 .stats-card.schools { background: linear-gradient(135deg, #400984ff, #fca5a5); }
  .stats-card.arms { background: linear-gradient(135deg, #079a1fff, #e4ceceff); }
   .stats-card.departments { background: linear-gradient(135deg, #c0c020ff, #dedcdcff); }
    /* Chart Card Styling */
    .chart-card {
        background: #f9fafb;
...
<div class="container">
    ...
    <div class="row">
         <div class="col-md-3 mb-3">
            <div class="card shadow-sm p-3 stats-card schools">
                <h5>Schools</h5>
                <h3>{{ $stats['schools'] }}</h3>
        ...
        <div class="col-md-3 mb-3">
            <div class="card shadow-sm p-3 stats-card arms">
                <h5>Total Arms</h5>
                <h3>{{ $stats['arms'] }}</h3>
                <small class="text-success">{{ $stats['active_arms'] }} active</small>
            </div>
        </div>
        <div class="col-md-3 mb-3">
            <div class="card shadow-sm p-3 stats-card departments">
                <h5>Total Departments</h5>
                <h3>{{ $stats['departments'] }}</h3>
                <small class="text-success">{{ $stats['active_departments'] }} active</small>
            </div>
        </div>
    </div>
    ...
    <div class="row mt-4">
        <div class="col-12 mb-3">
            <select id="chartDataSelector" class="form-select chart-selector">
                ...
                <option value="classes">User by Classes</option>
                <option value="groups">User by Group</option>
                 <option value="departments">User by Department</option>
                  <option value="arms">User by Arm</option>
                  <option value="categories">User by Category</option>
            </select>
        </div>
...
<script>
    ...
    const dataSets = {
        ...
        classes: {
            labels: {!! json_encode($stats['userClassData']->pluck('name'), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP) !!},
            data: {!! json_encode($stats['userClassData']->pluck('user_count'), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP) !!},
            title: 'User by Classes'
        },
        categories: {
            labels: {!! json_encode($stats['userCategoryData']->pluck('name'), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP) !!},
            data: {!! json_encode($stats['userCategoryData']->pluck('user_count'), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP) !!},
            title: 'User by Category'
        },
        groups: {
            labels: {!! json_encode($stats['userGroupsData']->pluck('name'), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP) !!},
            data: {!! json_encode($stats['userGroupsData']->pluck('user_count'), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP) !!},
            title: 'Users by Group'
        },
          arms: {
            labels: {!! json_encode($stats['userArmData']->pluck('name'), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP) !!},
            data: {!! json_encode($stats['userArmData']->pluck('user_count'), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP) !!},
            title: 'User by Arm'
        },
        departments: {
            labels: {!! json_encode($stats['userDepartmentData']->pluck('name'), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP) !!},
            data: {!! json_encode($stats['userDepartmentData']->pluck('user_count'), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP) !!},
